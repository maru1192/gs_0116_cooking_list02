...
// 既存の画像パスを取得（画像が変更されない場合のため）
$pdo = db_conn();
$stmt = $pdo->prepare("SELECT image FROM recipe_list WHERE recipe_id = :recipe_id");
$stmt->bindValue(':recipe_id', $recipe_id, PDO::PARAM_INT);
$stmt->execute();
$existing_data = $stmt->fetch(PDO::FETCH_ASSOC);
$image_path = $existing_data['image'] ?? null; // デフォルトは既存の画像パス
...
try {
    ...
    // recipe_listテーブルに登録
    $stmt = $pdo->prepare("UPDATE recipe_list 
                                SET 
                                    uid = :uid, 
                                    title = :title, 
                                    image = :image, 
    ...
    // 完了画面へリダイレクト
    header("Location: update_complete.php?id=" . $recipe_id);
    exit;
